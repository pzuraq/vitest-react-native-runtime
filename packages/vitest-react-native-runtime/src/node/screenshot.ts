...
export function detectPlatform(): Platform {
  ...
  if (android && ios) {
    throw new Error(
      'Both Android and iOS devices are running. Specify --platform android or --platform ios.',
    );
  }
  if (!android && !ios) {
    throw new Error(
...
export function captureScreenshot(options: ScreenshotOptions): ScreenshotResult {
  ...
  if (options.output) {
    ...
    const dir = options.outputDir ?? resolve(process.cwd(), '.vitest-native', 'screenshots');
    ensureScreenshotDir(dir);
    filePath = resolve(dir, `${name}-${timestamp}.png`);
  }

  if (platform === 'android') {
    captureAndroid(filePath);
  } else {
    captureIOS(filePath);
  }
...
function captureAndroid(filePath: string): void {
  try {
    ...
    throw new Error(
      `Failed to capture Android screenshot: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}

function captureIOS(filePath: string): void {
  try {
    ...
    throw new Error(
      `Failed to capture iOS screenshot: ${e instanceof Error ? e.message : String(e)}`,
    );
  }
}
